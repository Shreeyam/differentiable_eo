...
def plot_soft_visibility(path):
    ...
    ax.plot(x, hard, '--', lw=1.5, label='Step', color=C0)
    ax.plot(x, sigmoid, '-', lw=1.5, label=r'$\tau=2$', color=C1)
...
def plot_noisy_or(path):
    ...
    for c2, color, label in [(0.0, C2, r'$c_2=0$'),
                              (0.3, C1, r'$c_2=0.3$'),
                              (0.7, C0, r'$c_2=0.7$')][::-1]:
...
def plot_leaky_integrator(path):
    ...
    ax.plot(t, gap, '-', lw=1.5, color=C1)
    ax.plot(coverage_t, [0, 0], '^', ms=5, label='Coverage event', color=C0)
...
def plot_logsumexp(path):
    ...
    ax.plot(tau, lse, '-', lw=1.5, label='LSE', color=C1)
    ax.axhline(45, color=C0, ls='--', lw=1.5, label='True max')
